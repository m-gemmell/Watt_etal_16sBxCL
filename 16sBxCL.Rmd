...
https://github.com/m-gemmell-uoa/Watt_etal_16sBxCL


## Figure 1: Species diversity comparison between colonic biopsy and lavage sample

### Wilcox test to compare the Alpha diveristy values of Biopsy and Lavagae samples
```{r Alpha_diveristy_wilcox_test}
...
### Box plots for alpha diveristy value comparisons
...
#Change name of alpha diveristy values
colnames(plot_data_metadata)[1:5] <- c("Observed OTUs", "Chao","Shannon-Weiner","Inverse Simpson", "Coverage")
#Convert to long list format
alpha_long <- gather(plot_data_metadata, Alpha_diveristy_measure, value, 1:5)
#Reorder measures
alpha_long$Alpha_diveristy_measure_f <- factor(alpha_long$Alpha_diveristy_measure,levels=c("Observed OTUs", "Chao","Shannon-Weiner","Inverse Simpson", "Coverage"))
#Set colours
fa_b_colour <- brewer.pal(9, "Set1")
#Box Plot for alpha diversity comparing lavage against biopsy
g <- ggplot(alpha_long, aes(x=new.Sample.type, y=value, fill=new.Sample.type)) + geom_boxplot(outlier.colour = NA) + theme_set(theme_gray(base_size = 8)) +facet_wrap(~ Alpha_diveristy_measure_f, nrow=1, scales="free") + geom_point(position = position_jitter(width = 0.2)) + scale_x_discrete(breaks=NULL, name="") + scale_fill_manual(values=c(fa_b_colour[2], fa_b_colour[1]), name="Sample type") + scale_y_continuous(name="") + theme(legend.position="bottom", legend.margin=unit(0,"cm"), plot.margin =unit(c(0,0,0,0),"mm"))
ggsave("Fig1.species_diversity_between_colonic_and_lavage_samples.pdf", g, units="mm", height=120, width=170, dpi=300)
```
```{r alpha_diveristy_boxplot_figure, echo=FALSE}
...
plot_data_metadata <- plot_data_metadata[,c(-3,-4,-6,-7,-9,-10)]
#Change name of alpha diveristy scores
colnames(plot_data_metadata)[1:5] <- c("Observed OTUs", "Chao","Shannon-Weiner", "Inverse Simpson", "Coverage")
#Convert to long
alpha_long <- gather(plot_data_metadata, Alpha_diveristy_measure, value, 1:5)
#Reorder measures
alpha_long$Alpha_diveristy_measure_f <- factor(alpha_long$Alpha_diveristy_measure,
                     levels=c("Observed OTUs", "Chao","Shannon-Weiner", 
                              "Inverse Simpson", "Coverage"))
#p values for alpha diveristies
pvalues <- data.frame(Alpha_diveristy_measure_f = c("sobs",	"chao",	"shannon",	"invsimpson",	"coverage"),pvalue = c("4.213e-06",	"6.593e-10",	"0.7441",	"0.8278",	"2.223e-10"))
#Set colours
fa_b_colour <- brewer.pal(9, "Set1")
...
```
```{r phyla_relabund_piechart_fig, echo=FALSE}
if (!require("ggplot2")) {install.packages("ggplot2")
  library(ggplot2)}
if (!require("RColorBrewer")) {install.packages("RColorBrewer")
  library(RColorBrewer)}
...
dev.off()
```
```{r phyla_heatmap_fig, echo=FALSE}
### Load the package or install if not present
if (!require("arules")) {install.packages("arules")
  library(arules)}
if (!require("ade4")) {install.packages("ade4")
...
#Selectinging a colour palette to be used in figure
brewer_colours <- brewer.pal(9, "Set1")
...
otu_info <- otu_info[order(row.names(otu_info)),]
metadata_info <- metadata_info[order(row.names(metadata_info)),]
identical(row.names(metadata_info), row.names(otu_info))
row.names(otu_info) <- metadata_info$new.Sample.ID
otu_info <- otu_info[order(row.names(otu_info)),]
#
trans_otu_info <- t(otu_info)
...
spearman_cor <- data_rcorr$r
spearman_p <- data_rcorr$P
#Make tables have only data I am interested in
subset_spearman_cor <- spearman_cor[1:23,24:46]
subset_spearman_p <- spearman_p[1:23,24:46]
#Correlation plot of FA samples against bx samples
par(mfrow=c(1,1), mar=c(0,0,0,0), cex=0.6)
...
#To determine order of alpha diveristy for plot
...
#Convert to long
alpha_long <- gather(plot_data_metadata, Alpha_diveristy_measure, value, 2:(ncol(ALPHA_DATA)+1))
#Reorder measures
alpha_long$Alpha_diveristy_measure_f <- factor(alpha_long$Alpha_diveristy_measure, levels=ORDER_ALPHA)
return(alpha_long)
}
...
mann_whitney_data <- spread(alpha_long_t, Alpha_diveristy_measure_f, value)
...
#Select colour palette for plot
col.brew <- c(brewer.pal(9, "Set1"),brewer.pal(8,"Set2"),brewer.pal(12,"Set3"))
#Box Plot for alpha diveristy comparing aspirate and biopsy
g_box <- ggplot(alpha_long, aes(x=Sample.type, y=value, fill=new.Sample.type)) +  geom_boxplot(outlier.colour = NA) + theme_set(theme_gray(base_size = 8)) + facet_wrap(~ Alpha_diveristy_measure_f, nrow=1, scales="free") + geom_point(position = position_jitter(width = 0.2)) + scale_x_discrete(breaks=NULL, name="") + scale_fill_manual(values=c(col.brew[2], col.brew[1]), name="Sample type") + scale_y_continuous(name="") + theme_grey(base_size = 8) + theme(legend.position="bottom", legend.margin=unit(0,"cm"))
...
alpha_data <- alpha_data[alpha_data$method == "ave",]
rownames(alpha_data) <- alpha_data$group
#Remove the columns you are not interested in the alpha diveristy data frame
alpha_data <- alpha_data[,c("sobs", "chao", "shannon", "invsimpson", "coverage")]
#Rename columns
colnames(alpha_data) <- c("KEGG Pathways", "Chao","Shannon-Weiner", "Inverse Simpson", "Coverage")
#To determine order of alpha diveristy for plot
